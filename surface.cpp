...
{
	x1 = max(min(x1, m_Width), 0);
	x2 = max(min(x2, m_Width), 0);
	y1 = max(min(y1, m_Height), 0);
	y2 = max(min(y2, m_Height), 0);
...
//CODE FROM CHRISTIAAN BLOEMENDAAL
void Surface::AlphaCopyTo( Surface* a_Dst, int a_X, int a_Y )
{
		Pixel* dst = a_Dst->GetBuffer();
		Pixel* src = m_Buffer;
		if ((src) && (dst))
